...
import { DismissibleNotice, FloatingActionButton, Icon, IconButton } from "@/design-system";
...
export function HomeView({ date, recurrencesGenerated = false, onboardingDone = false }: { date: string; recurrencesGenerated?: boolean; onboardingDone?: boolean }) {
  ...
  return <Screen eyebrow={date} title={`Bon dia, ${data.people[0]?.name ?? "!"}`} action={<Link href="/configuracio"><IconButton label="Configuració" variant="secondary"><Icon name="settings" /></IconButton></Link>}>
    {showGeneratedMessage && <DismissibleNotice id="recurrences-generated" title="Moviments recurrents generats" message="Els saldos, moviments i estadístiques ja estan actualitzats." tone="success" />}
    {showWelcome && <DismissibleNotice id="onboarding-complete" title="Configuració completada" message="Ja pots començar a utilitzar Comptes." tone="success" />}
    {backupReminder && <DismissibleNotice id="backup-reminder" title="Convé fer una còpia de seguretat" message="Les dades només són en aquest dispositiu." tone="warning" onDismiss={() => { const later = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(); localStorage.setItem("comptes-backup-reminder-after", later); setBackupReminder(false); }}><Link className={styles.reviewButton} href="/configuracio/copies-de-seguretat">Fer còpia</Link></DismissibleNotice>}
    {pending.length > 0 && <DismissibleNotice id={`pending-recurrences-${new Date().toISOString().slice(0, 7)}`} version={pending.length} persistence="local" title="Tens moviments recurrents pendents" message={`${pending.length} moviments preparats per revisar i generar aquest mes.`} tone="warning"><Link className={styles.reviewButton} href="/recurrencies-pendents">Revisar</Link></DismissibleNotice>}
