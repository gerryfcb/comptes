...
import { BottomSheet, Button, Icon, Input } from "@/design-system";
...
export function MovementForm({ title, action, kind, movementId }: { title: string; action: string; kind: MovementKind; movementId?: string }) {
  ...
  function activateCategory(libraryId: string) {
    const item = CATEGORY_LIBRARY.find((entry) => entry.id === libraryId); if (!item) return;
    const existing = data.categories.find((entry) => entry.name.toLocaleLowerCase("ca") === item.name.toLocaleLowerCase("ca"));
    update((current) => activateLibraryCategories(current,[libraryId])); setCategoryId(existing?.id ?? `library-${libraryId}`); setCategorySheet(false);
  }
  function createCustomCategory() {
    ...
    if (existing) { setCategoryId(existing.id); setCategorySheet(false); return; }
    const id = newId("cat"); update((current) => ({ ...current, categories:[...current.categories,{ id, name, kind:kind === "income" ? "income" : "expense", icon:kind === "income" ? "income" : "shopping", color:customColor, order:current.categories.length + 1, archived:false }] })); setCategoryId(id); setCategorySheet(false); setCustomCategory("");
  }
  function createSubcategory() {
    const name = subcategoryName.trim(); if (!name || !categoryId) return;
    const id = newId("sub"); update((current) => ({ ...current, subcategories:[...current.subcategories,{ id, categoryId, name, order:current.subcategories.filter((item) => item.categoryId === categoryId).length + 1, archived:false }] })); setSubcategoryId(id); setSubcategoryName("");
  }
  ...
  return <Screen title={title} eyebrow={movement ? "Edita el moviment" : "Dades locals"} backHref="/moviments"><form className={styles.form} onSubmit={submit}>
    ...
    {(kind === "expense" || kind === "income") && <><label className={styles.selectField}>Categoria {kind === "income" && "(opcional)"}<select name="categoryId" value={categoryId} onChange={(e) => { setCategoryId(e.target.value); setSubcategoryId(""); }}><option value="">Sense categoria</option>{categories.map((c) => <option value={c.id} key={c.id}>{c.name}{c.archived ? " · arxivada" : ""}</option>)}</select></label><Button type="button" variant="secondary" onClick={() => setCategorySheet(true)}>+ Crear categoria o explorar biblioteca</Button></>}
    {kind === "expense" && <><label className={styles.selectField}>Subcategoria<select name="subcategoryId" value={subcategoryId} onChange={(event) => setSubcategoryId(event.target.value)}><option value="">Sense subcategoria</option>{subs.map((s) => <option value={s.id} key={s.id}>{s.name}{s.archived ? " · arxivada" : ""}</option>)}</select></label>{categoryId && <div className={styles.inlineActions}><Input label="Nova subcategoria" value={subcategoryName} onChange={(event) => setSubcategoryName(event.target.value)} /><Button type="button" variant="secondary" onClick={createSubcategory}>+ Crear subcategoria</Button></div>}</>}
  ...
  </form><BottomSheet open={categorySheet} title="Categories" description="Activa’n una de la biblioteca o crea’n una de personalitzada." onClose={() => setCategorySheet(false)}><div className={styles.libraryGrid}>{CATEGORY_LIBRARY.filter((item) => item.kind === (kind === "income" ? "income" : "expense")).map((item) => <button key={item.id} onClick={() => activateCategory(item.id)}><Icon name={item.icon} /><span>{item.name}<small>{item.subcategories.length} subcategories</small></span></button>)}</div><div className={styles.form}><Input label="Categoria personalitzada" value={customCategory} onChange={(event) => setCustomCategory(event.target.value)} /><label className={styles.selectField}>Color<select value={customColor} onChange={(event) => setCustomColor(event.target.value)}>{COLOR_PALETTE.map(([value,label]) => <option value={value} key={value}>{label}</option>)}</select></label><Button onClick={createCustomCategory}>Crear i seleccionar</Button></div></BottomSheet></Screen>;
}
