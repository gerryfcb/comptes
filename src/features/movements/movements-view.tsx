...
const actions = [["despesa", "Nova despesa", "shopping"], ["ingres", "Nou ingrés", "income"], ["transferencia", "Nova transferència", "transfer"], ["estalvi", "Moviment d’estalvi", "saving"]] as const;
...
export function MovementsView() {
  ...
  const [query, setQuery] = useState(""); const [type, setType] = useState(""); const [account, setAccount] = useState(""); const [category, setCategory] = useState(""); const [sheet, setSheet] = useState(false);
  ...
  return <Screen title="Moviments" eyebrow="Tota la teva activitat" action={<Button leadingIcon={<Icon name="add" />} onClick={() => setSheet(true)}>Afegir</Button>}>
    ...
    <BottomSheet open={sheet} title="Nou moviment" description="Què vols registrar?" onClose={() => setSheet(false)}><div className={styles.settingsList}>{actions.map(([href, label, icon]) => <Link className={styles.settingsLink} href={`/moviments/nou/${href}`} key={href}><span><Icon name={icon} /></span><div><b>{label}</b><small>Desa’l al dispositiu</small></div><Icon name="chevron-right" /></Link>)}</div></BottomSheet>
  </Screen>;
}
