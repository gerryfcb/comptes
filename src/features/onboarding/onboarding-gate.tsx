  ...
  return <Screen title={titles[step - 1]} eyebrow={`Pas ${step} de 6`}>
    ...
    {step === 3 && <article className={styles.wizardCard}><p>Els comptes serveixen per saber d&apos;on surten i on són els diners.</p><div className={styles.libraryGrid}>{presets.map((preset) => { const selected = accounts.some((item) => item.preset === preset.id); return <button key={preset.id} className={selected ? styles.choiceActive : ""} onClick={() => togglePreset(preset.id)}><Icon name={preset.icon} />{preset.name}</button>; })}</div>{accounts.map((account) => {
      ...
      return <div className={styles.accountDraft} key={account.preset}>
        ...
        {hasMultipleOwners && <div className={styles.noticeInline}><b>Quina part d’aquest compte forma part del teu patrimoni?</b><p>Aquest percentatge només serveix per calcular la teva part del patrimoni. No modifica el saldo real del compte.</p><div className={styles.quickGrid}>{shareOptions.map((option) => <button type="button" className={!customShares.includes(account.preset) && account.attributablePercentage === option ? styles.choiceActive : ""} key={option} onClick={() => { updateAccount(account.preset, { attributablePercentage: option }); setCustomShares((items) => items.filter((id) => id !== account.preset)); }}>{option}%</button>)}<button type="button" className={customShares.includes(account.preset) ? styles.choiceActive : ""} onClick={() => setCustomShares((items) => [...new Set([...items, account.preset])])}>Altre</button></div>{customShares.includes(account.preset) && <label className={styles.rangeField}><span>Percentatge personalitzat <strong>{account.attributablePercentage}%</strong></span><input aria-label={`Percentatge de ${account.name}`} type="range" min="0" max="100" step="1" value={account.attributablePercentage} onChange={(event) => updateAccount(account.preset, { attributablePercentage: Number(event.target.value) })} /></label>}</div>}
