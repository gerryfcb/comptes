...
export function AccountForm({ title, action, accountId }: { title: string; action: string; accountId?: string }) {
  ...
  const [customShare, setCustomShare] = useState(Boolean(account && !shareOptions.includes(account.attributablePercentage)));
  ...
  return (
    <Screen title={title} eyebrow={account ? "Edita les dades del compte" : "Nou compte"} backHref="/comptes">
      <form className={styles.form} onSubmit={submit}>
        ...
        {multipleOwners && <article className={styles.noticeInline}><b>Quina part d’aquest compte forma part del teu patrimoni?</b><p>Aquest percentatge només serveix per calcular la teva part del patrimoni. No modifica el saldo real del compte.</p><div className={styles.quickGrid}>{shareOptions.map((option) => <button className={!customShare && share === option ? styles.choiceActive : ""} type="button" key={option} onClick={() => { setShare(option); setCustomShare(false); }}>{option}%</button>)}<button className={customShare ? styles.choiceActive : ""} type="button" onClick={() => setCustomShare(true)}>Altre</button></div>{customShare && <label className={styles.rangeField}><span>Percentatge personalitzat <strong>{share}%</strong></span><input aria-label="Percentatge personalitzat" type="range" min="0" max="100" step="1" value={share} onChange={(event) => setShare(Number(event.target.value))} /></label>}</article>}
